...
import tech.jmcs.floortech.scheduling.ui.data.table.*;
...
public class DataFramePresenter implements Initializable {
    ...
    public void initialize(URL location, ResourceBundle resources) {
        LOG.debug("DataFramePresenter initialized");
    ...
    private void setupListenerToExtractedData() {
        this.extractedDataHolder.lastUpdatedProperty().addListener((observable, oldValue, newValue) -> {
            if (newValue.after(oldValue)) {
                ...
                if (this.extractedDataHolder.getCustomDataList() != null && !this.extractedDataHolder.getCustomDataList().isEmpty()) {
    ...
    private void installCustomDataTables() {
        List<Map<Long, Map<String, Object>>> customDataList = this.extractedDataHolder.getCustomDataList();
        // Long is Id, Map<String, Object> is name:value

        // iterate each custom data collection
        for (Map<Long, Map<String, Object>> dataMap : customDataList) {
